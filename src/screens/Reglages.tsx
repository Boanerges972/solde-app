...
// ── SETTINGS ─────────────────────────────────────────────────
export const Settings = ({ t, user, onLogout, profile, onProfile, onSecurity, onRecurring, onReset, onGroupe, themeMode, onThemeMode, onBudgets, onGoals, onRules, onReport }: SettingsProps) => (
  <div style={{ padding: '0 20px 16px' }}>
    ...
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ background: t.card, borderRadius: 16, border: '1px solid ' + t.bo, overflow: 'hidden' }}>
        {onBudgets && (
          <button onClick={onBudgets}
            style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%',
              padding: '14px 16px', background: 'none', border: 'none', borderBottom: '1px solid ' + t.bo, cursor: 'pointer', textAlign: 'left' }}>
            <div style={{ width: 36, height: 36, borderRadius: 11, background: t.mD,
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>🎯</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, ...sp('o', 500), color: t.tx }}>Budgets par catégorie</div>
              <div style={{ fontSize: 11, ...sp('o'), color: t.sub, marginTop: 1 }}>
                Enveloppes mensuelles, jauges, report
              </div>
            </div>
        ...
        {onReport && (
          <button onClick={onReport}
            ...
            <div style={{ width: 36, height: 36, borderRadius: 11, background: t.el,
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>📊</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, ...sp('o', 500), color: t.tx }}>Rapport mensuel</div>
              <div style={{ fontSize: 11, ...sp('o'), color: t.sub, marginTop: 1 }}>
                Synthèse imprimable / PDF du mois
              </div>
            </div>
            <span style={{ color: t.muted, fontSize: 16 }}>›</span>
          </button>
        ...
        {onRules && (
          <button onClick={onRules}
            ...
            <div style={{ width: 36, height: 36, borderRadius: 11, background: t.el,
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>🧠</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, ...sp('o', 500), color: t.tx }}>Mes règles de catégorie</div>
              <div style={{ fontSize: 11, ...sp('o'), color: t.sub, marginTop: 1 }}>
                Apprises de tes saisies, appliquées aux imports
              </div>
            </div>
            <span style={{ color: t.muted, fontSize: 16 }}>›</span>
          </button>
        ...
        {onGoals && (
          <button onClick={onGoals}
            ...
            <div style={{ width: 36, height: 36, borderRadius: 11, background: t.el,
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>💰</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, ...sp('o', 500), color: t.tx }}>Objectifs d'épargne</div>
              <div style={{ fontSize: 11, ...sp('o'), color: t.sub, marginTop: 1 }}>
                Cibles, échéances, progression
              </div>
            </div>
            <span style={{ color: t.muted, fontSize: 16 }}>›</span>
          </button>
